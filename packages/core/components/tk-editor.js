...
function prepareSliceForReplace(slice, $along) {
  ...
  return { start: node.resolveNoCache(slice.openStart + extra), end: node.resolveNoCache(node.content.size - slice.openEnd - extra) };
}
...
class ParseContext {
  ...
  addAll(parent, marks, startIndex, endIndex) {
    ...
    for (
      let dom = startIndex ? parent.childNodes[startIndex] : parent.firstChild, end = endIndex == null ? null : parent.childNodes[endIndex];
      dom != end;
...
class ReplaceAroundStep extends Step {
  ...
  toJSON() {
    let json = { stepType: 'replaceAround', from: this.from, to: this.to, gapFrom: this.gapFrom, gapTo: this.gapTo, insert: this.insert };
    if (this.slice.size) json.slice = this.slice.toJSON();
    if (this.structure) json.structure = true;
    return json;
  }
    ...
    for (let pass = 1; pass <= 2; pass++) {
      for (let sliceDepth = pass == 1 ? startDepth : this.unplaced.openStart; sliceDepth >= 0; sliceDepth--) {
        ...
        for (let frontierDepth = this.depth; frontierDepth >= 0; frontierDepth--) {
          ...
          // `first` fit here.
          else if (pass == 2 && first && (wrap = match.findWrapping(first.type))) return { sliceDepth, frontierDepth, parent, wrap };
      ...
      for (let i = 0; i < this.config.plugins.length; i++) {
        ...
        if (plugin.spec.appendTransaction) {
          ...
          if (tr && newState.filterTransaction(tr, i)) {
            ...
            if (!seen) {
              seen = [];
              for (let j = 0; j < this.config.plugins.length; j++) seen.push(j < i ? { state: newState, n: trs.length } : { state: this, n: 0 });
            }
            trs.push(tr);
...
class ViewDesc {
  ...
  parseRange(from, to, base = 0) {
    if (this.children.length == 0) return { node: this.contentDOM, from, to, fromOffset: 0, toOffset: this.contentDOM.childNodes.length };
    let fromOffset = -1,
      toOffset = -1;
        ...
        if (brKludge && offset == node.nodeValue.length) {
          for (let scan = node, after; scan; scan = scan.parentNode) {
            if ((after = scan.nextSibling)) {
              if (after.nodeName == 'BR') anchorDOM = headDOM = { node: after.parentNode, offset: domIndex(after) + 1 };
              break;
            }
    ...
    for (let i = this.index; i < this.top.children.length; i++) {
      ...
      if (next instanceof NodeViewDesc) {
        ...
        let locked =
          ...
          !(node.isText && next.node && next.node.isText && next.nodeDOM.nodeValue == node.text && next.dirty != NODE_DIRTY && sameOuterDeco(outerDeco, next.outerDeco));
...
function ensureListeners(view) {
  view.someProp('handleDOMEvents', currentHandlers => {
    for (let type in currentHandlers)
      if (!view.input.eventHandlers[type]) view.dom.addEventListener(type, (view.input.eventHandlers[type] = event => runCustomHandler(view, event)));
  });
}
...
function handleTripleClick(view, pos, inside, event) {
  return (
    runHandlerOnContext(view, 'handleTripleClickOn', pos, inside, event) || view.someProp('handleTripleClick', f => f(view, pos, event)) || defaultTripleClick(view, inside, event)
  );
}
...
class MouseDown {
  constructor(view, pos, event, flushed) {
    ...
    if (
      (event.button == 0 && targetNode.type.spec.draggable && targetNode.type.spec.selectable !== false) ||
...
class WidgetType {
  ...
  eq(other) {
    return (
      this == other || (other instanceof WidgetType && ((this.spec.key && this.spec.key == other.spec.key) || (this.toDOM == other.toDOM && compareObjs(this.spec, other.spec))))
    );
  }
  destroy(node) {
...
class DecorationSet {
  ...
  eq(other) {
    ...
    for (let i = 0; i < this.children.length; i += 3)
      if (this.children[i] != other.children[i] || this.children[i + 1] != other.children[i + 1] || !this.children[i + 2].eq(other.children[i + 2])) return false;
    return true;
  }
  /**
    @internal
...
class DOMObserver {
    ...
    this.observer =
      ...
      new window.MutationObserver(mutations => {
        ...
        if (
          ie$1 &&
          ie_version <= 11 &&
          mutations.some(m => (m.type == 'childList' && m.removedNodes.length) || (m.type == 'characterData' && m.oldValue.length > m.target.nodeValue.length))
        )
          this.flushSoon();
        else this.flush();
      });
  ...
  registerMutation(mut, added) {
    ...
    if (mut.type == 'childList') {
      ...
      if (desc.contentDOM && desc.contentDOM != desc.dom && !desc.contentDOM.contains(mut.target)) return { from: desc.posBefore, to: desc.posAfter };
...
function readDOMChange(view, from, to, typeOver, addedNodes) {
  ...
  if (
    ((ios && view.input.lastIOSEnter > Date.now() - 225 && (!inlineChange || addedNodes.some(n => n.nodeName == 'DIV' || n.nodeName == 'P'))) ||
      (!inlineChange &&
...
class EditorView {
  ...
  updateStateInner(state, prevProps) {
    ...
    if (updateSel) {
      ...
      // #1011, #1013, #1035).
      let forceSelUpdate =
        updateDoc && (ie$1 || chrome) && !this.composing && !prev.selection.empty && !state.selection.empty && selectionContextChanged(prev.selection, state.selection);
      if (updateDoc) {
...
function keyName(event) {
  ...
  var ignoreKey =
    (mac$1 && event.metaKey && event.shiftKey && !event.ctrlKey && !event.altKey) || (ie && event.shiftKey && event.key && event.key.length == 1) || event.key == 'Unidentified';
    ...
    for (let depth = $cursor.depth; ; depth--) {
      ...
      if (delStep && delStep.slice.size < delStep.to - delStep.from) {
        if (dispatch) {
          ...
          tr.setSelection(
            textblockAt(before, 'end') ? Selection.findFrom(tr.doc.resolve(tr.mapping.map($cut.pos, -1)), -1) : NodeSelection.create(tr.doc, $cut.pos - before.nodeSize),
          );
...
function getAttributesFromExtensions(extensions) {
  ...
  nodeAndMarkExtensions.forEach(extension => {
    ...
    Object.entries(attributes).forEach(([name, attribute]) => {
      ...
      if (
        (mergedAttr === null || mergedAttr === void 0 ? void 0 : mergedAttr.isRequired) &&
        (mergedAttr === null || mergedAttr === void 0 ? void 0 : mergedAttr.default) === undefined
      ) {
        ...
        attrs: Object.fromEntries(
          extensionAttributes.map(extensionAttribute => {
            ...
            return [
              ...
              {
                default:
                  (_a = extensionAttribute === null || extensionAttribute === void 0 ? void 0 : extensionAttribute.attribute) === null || _a === void 0 ? void 0 : _a.default,
              },
            ];
          }),
        ),
    ...
    const coreExtensions = this.options.enableCoreExtensions
      ? [
          ...
          ClipboardTextSerializer.configure({
            blockSeparator:
              (_b = (_a = this.options.coreExtensionOptions) === null || _a === void 0 ? void 0 : _a.clipboardTextSerializer) === null || _b === void 0
                ? void 0
                : _b.blockSeparator,
          }),
          Commands,
        ...
        ({ commands, chain }) => {
          if (this.options.keepAttributes) {
            return chain()
              ...
              .updateAttributes(ListItemName$1, this.editor.getAttributes(TextStyleName$1))
              .run();
          }
...
RopeSequence.prototype.append = function append(other) {
  ...
  return (
    (!this.length && other) || (other.length < GOOD_LEAF_SIZE && this.leafAppend(other)) || (this.length < GOOD_LEAF_SIZE && other.leafPrepend(this)) || this.appendInner(other)
  );
};
...
function applyTransaction(history, state, tr, options) {
  ...
  if (tr.steps.length == 0) {
    ...
    let newGroup =
      history.prevTime == 0 ||
      (!appended && history.prevComposition != composition && (history.prevTime < (tr.time || 0) - options.newGroupDelay || !isAdjacentTo(tr, history.prevRanges)));
    let prevRanges = appended ? mapRanges(history.prevRanges, tr.mapping) : rangesFor(tr.mapping.maps);
    return new HistoryState(
      history.done.addTransform(tr, newGroup ? state.selection.getBookmark() : undefined, options, mustPreserveItems(state)),
      Branch.empty,
      prevRanges,
      tr.time,
...
function stringToArray(str) {
  ...
  while (index < len) {
    ...
    let second;
    let char =
      first < 0xd800 || first > 0xdbff || index + 1 === len || (second = str.charCodeAt(index + 1)) < 0xdc00 || second > 0xdfff
...
const TkEditor$1 = /*@__PURE__*/ proxyCustomElement(
  class TkEditor extends H {
    constructor() {
      ...
      this.handleToolbarAction = button => {
        ...
        const commands = {
          undo: () => {
            var _a;
            return (_a = this.command()) === null || _a === void 0 ? void 0 : _a.undo();
          },
          redo: () => {
            var _a;
            return (_a = this.command()) === null || _a === void 0 ? void 0 : _a.redo();
          },
          heading: () => {
            var _a;
            return (_a = this.command()) === null || _a === void 0 ? void 0 : _a.toggleHeading({ level: button.level });
          },
          bold: () => {
            var _a;
            return (_a = this.command()) === null || _a === void 0 ? void 0 : _a.toggleBold();
          },
          italic: () => {
            var _a;
            return (_a = this.command()) === null || _a === void 0 ? void 0 : _a.toggleItalic();
          },
          underline: () => {
            var _a;
            return (_a = this.command()) === null || _a === void 0 ? void 0 : _a.toggleUnderline();
          },
          strike: () => {
            var _a;
            return (_a = this.command()) === null || _a === void 0 ? void 0 : _a.toggleStrike();
          },
          clearFormat: () => {
            var _a;
            return (_a = this.command()) === null || _a === void 0 ? void 0 : _a.unsetAllMarks().clearNodes().setParagraph();
          },
          bulletList: () => {
            var _a;
            return (_a = this.command()) === null || _a === void 0 ? void 0 : _a.toggleBulletList();
          },
          orderedList: () => {
            var _a;
            return (_a = this.command()) === null || _a === void 0 ? void 0 : _a.toggleOrderedList();
          },
          alignLeft: () => {
            var _a;
            return (_a = this.command()) === null || _a === void 0 ? void 0 : _a.setTextAlign('left');
          },
          alignCenter: () => {
            var _a;
            return (_a = this.command()) === null || _a === void 0 ? void 0 : _a.setTextAlign('center');
          },
          alignRight: () => {
            var _a;
            return (_a = this.command()) === null || _a === void 0 ? void 0 : _a.setTextAlign('right');
          },
          alignJustify: () => {
            var _a;
            return (_a = this.command()) === null || _a === void 0 ? void 0 : _a.setTextAlign('justify');
          },
          link: () => {
            var _a;
    ...
    get activeExtensions() {
      ...
      const starterKitExclusions = STARTER_KIT_EXTENSION_NAMES.filter(name => userExtensionNames.has(name)).reduce(
        (acc, name) => Object.assign(Object.assign({}, acc), { [name]: false }),
        {},
      );
    ...
    renderToolbar() {
      ...
      return h(
        'div',
        { class: 'tk-editor-toolbar' },
        h(
          'div',
          { class: 'tk-editor-controlsgroup-container' },
          toolbarConfig.map((group, groupIndex) =>
            h(
              'div',
              { class: 'tk-editor-controlsgroup', key: groupIndex },
              group.map(button => {
    ...
    renderHint() {
      ...
      if (((_a = this.hint) === null || _a === void 0 ? void 0 : _a.length) > 0) {
        hint = h(
          'div',
          { class: 'tk-editor-supporting-text' },
          h('tk-icon', Object.assign({}, getIconElementProps('info', { class: classNames('tk-editor-supporting-text-icon'), variant: null }))),
          h('span', null, this.hint),
        );
      }
      if (((_b = this.error) === null || _b === void 0 ? void 0 : _b.length) > 0) {
        hint = h(
          'div',
          { class: 'tk-editor-supporting-text error' },
          h('tk-icon', Object.assign({}, getIconElementProps('info', { class: classNames('tk-editor-supporting-text-icon'), variant: null }))),
    ...
    render() {
      ...
      const labelElement =
        ...
        h(
          'label',
          { key: '90372422e3a03031dabce3b9fa0cfc9097bb71db', class: 'tk-editor-label' },
          this.label,
          this.showAsterisk && h('span', { key: '7224d9839d2d224909903e6f5f4587857e33506f', class: 'tk-editor-label-asterisk' }, '*'),
        );
      return h(
        'div',
        {
          key: '26a3e7808a7c116e8a1c21b00f8daac2cf44baaf',
        ...
        h(
          'div',
          { key: 'd9090ece0916ca212b7f1a2340331d54b2ec24dc', class: 'tk-editor' },
          this.renderToolbar(),
          h('div', {
            'key': '4df75d0c78e0af3ec7c730a48f251c37cb68f6a1',
    ...
    static get watchers() {
      return {
        value: ['valueChanged'],
        disabled: ['editableChanged'],
        readonly: ['editableChanged'],
      };
    }
    static get style() {
      return tkEditorCss;
  ...
  [
    ...
    'tk-editor',
    {
      ...
      readonly: [4],
      placeholder: [1],
      hideToolbar: [4, 'hide-toolbar'],
      contentStyle: [8, 'content-style'],
      extensions: [16],
      customToolbarButtons: [16, 'custom-toolbar-buttons'],
      toolbar: [16],
      label: [1],
      hint: [1],
      invalid: [4],
...
function defineCustomElement$1() {
  ...
  const components = ['tk-editor', 'tk-icon'];
  components.forEach(tagName => {
    switch (tagName) {
      case 'tk-editor':
        if (!customElements.get(tagName)) {
          customElements.define(tagName, TkEditor$1);
        }
        break;
      case 'tk-icon':
        if (!customElements.get(tagName)) {
...
const TkEditor = TkEditor$1;
const defineCustomElement = defineCustomElement$1;

export { TkEditor, defineCustomElement };
//# sourceMappingURL=tk-editor.js.map
