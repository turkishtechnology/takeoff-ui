...
Users can click on the column headers to apply sorting, and the order of sorting can be adjusted by clicking on the headers in the desired sequence.
<TkAlert
  header="Information"
...
Customize pagination labels for different locales using `pageReportTemplate` and `itemsReportTemplate` props.
Available placeholders: `{currentPage}`, `{totalPages}`, `{startItem}`, `{endItem}`, `{totalItems}`.
