...
const ToggleButtonGroupVariants = () => {
  ...
  const [buttonType, setButtonType] = useState<'filled' | 'outlined' | 'text' | 'raised' | 'filled-light'>('outlined');
  ...
  const generateCode = () => {
    const attrs = [`value="${value}"`, `type="${groupType}"`, `direction="${direction}"`, rounded ? 'rounded' : '', `onTkChange={(e) => setValue(e.detail)}`]
      .filter(Boolean)
      .join('\n  ');
...
${buttonCode}
    ...
    const vueAttrs = [`:value="${value}"`, `type="${groupType}"`, `direction="${direction}"`, rounded ? ':rounded="true"' : '', `@tk-change="(e) => value = e.detail"`]
      .filter(Boolean)
      .join('\n  ');
...
${vueButtonCode}
    ...
    const angularAttrs = [`[value]="'${value}'"`, `type="${groupType}"`, `direction="${direction}"`, rounded ? '[rounded]="true"' : '', `(tkChange)="onChange($event)"`]
      .filter(Boolean)
      .join('\n  ');
...
>
  ...
  useEffect(() => {
    generateCode();
  }, [groupType, buttonType, buttonVariant, direction, rounded, value, disabled, singleButtonValue]);
    ...
    <div className="flex flex-col gap-8 items-start w-full">
      ...
      <div className="flex flex-col gap-4 w-full">
        ...
        <div className="flex flex-wrap gap-4">
          <TkRadioGroup label="Group Type" value={groupType} onTkChange={e => setGroupType(e.detail)}>
            {groupTypeOptions.map(t => (
              <TkRadio key={t} label={t.charAt(0).toUpperCase() + t.slice(1)} value={t} />
            ))}
          </TkRadioGroup>

          <TkRadioGroup label="Button Type" value={buttonType} onTkChange={e => setButtonType(e.detail)}>
            {buttonTypeOptions.map(t => (
              <TkRadio key={t} label={t.replace('-', ' ').replace(/\b\w/g, c => c.toUpperCase())} value={t} />
            ))}
          </TkRadioGroup>

          <TkRadioGroup label="Variant" value={buttonVariant} onTkChange={e => setButtonVariant(e.detail)}>
            {buttonVariantOptions.map(v => (
              <TkRadio key={v} label={v.charAt(0).toUpperCase() + v.slice(1)} value={v} />
            ))}
          </TkRadioGroup>

          <TkRadioGroup label="Direction" value={direction} onTkChange={e => setDirection(e.detail)}>
            {directionOptions.map(d => (
              <TkRadio key={d} label={d.charAt(0).toUpperCase() + d.slice(1)} value={d} />
            ))}
          </TkRadioGroup>

          <TkCheckbox label="Rounded" value={rounded} onTkChange={() => setRounded(prev => !prev)} />

          <TkCheckbox label="Disabled" value={disabled} onTkChange={() => setDisabled(prev => !prev)} />
        </div>
