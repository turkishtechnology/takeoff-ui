...
You can set the size of the dropdown using the `size` prop. The available sizes are `large` , `base`, and `small`. If no size is specified, the default size is `base`.
