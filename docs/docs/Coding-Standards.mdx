...
This document outlines comprehensive guidelines for component development using
the Stencil library. These standards ensure consistency and maintainability
across projects.
...
const element = await page.find(
  'tk-component >>> [data-testid="component-element"]',
);
...
The framework integration is handled through the Stencil configuration.
Component developers should ensure their components emit proper events for
framework bindings:
